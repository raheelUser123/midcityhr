...
<form class="form-card wizard-form" data-lead-form data-wizard-form>
  ...
  <section class="wizard-step active" data-step="1">
    ...
    <div class="choice-grid">
      <?php foreach(services() as $i=>$s): ?><label class="choice-card"><input type="radio" name="service" value="<?=e($s['name'])?>" <?=($form_service===$s['name'] || (!$form_service && $i===0))?'checked':''?>><span class="choice-icon"><?=['🛁','🍳','🏠','🛠️','✅','🌧️','🔨','📋'][$i]??'🔧'?></span><span><strong><?=e($s['name'])?></strong><small><?=e($s['summary'])?></small></span></label><?php endforeach;?>
    </div>
