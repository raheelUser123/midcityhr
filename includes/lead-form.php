...
<form class="form-card wizard-form" data-lead-form data-wizard-form>
  ...
  <section class="wizard-step active" data-step="1">
    ...
    <div class="choice-grid">
      <?php foreach(services() as $i=>$s): ?><label class="choice-card"><input type="radio" name="service" value="<?=e($s['name'])?>" <?=($form_service===$s['name'] || (!$form_service && $i===0))?'checked':''?>><span class="choice-icon" aria-hidden="true"><?=['🛁','🍳','🏠','🛠️','✅','🌧️','🔨','📋'][$i]??'🔧'?></span><span><strong><?=e($s['name'])?></strong><small><?=e($s['summary'])?></small></span></label><?php endforeach;?>
    </div>
  ...
  <section class="wizard-step" data-step="2">
    ...
    <div class="form-grid">
      ...
      <div class="field full"><label>How did you hear about us?</label><select name="referral_source"><option value="">Select an option</option><option value="Google Search">Google Search</option><option value="Google Maps">Google Maps</option><option value="Google Ads">Google Ads</option><option value="Facebook">Facebook</option><option value="Instagram">Instagram</option><option value="Online Directory / Contractor Website">Online Directory / Contractor Website</option><option value="Referral from a Friend or Family Member">Referral from a Friend or Family Member</option><option value="Previous MHR Customer">Previous MHR Customer</option><option value="Saw Our Yard Sign">Saw Our Yard Sign</option><option value="Saw Our Vehicle / Trailer">Saw Our Vehicle / Trailer</option><option value="Neighborhood / Saw Us Working Nearby">Neighborhood / Saw Us Working Nearby</option><option value="Home Show / Local Event">Home Show / Local Event</option><option value="Realtor / Property Manager">Realtor / Property Manager</option><option value="Contractor or Supplier Network">Contractor or Supplier Network</option><option value="Manufacturer / Brand Referral">Manufacturer / Brand Referral</option><option value="Other">Other, pls specify</option></select></div>
      <div class="field full" id="referral_other_wrap" style="display:none;"><label>Please specify *</label><input type="text" name="referral_other" placeholder="Please specify how you heard about us"></div>
